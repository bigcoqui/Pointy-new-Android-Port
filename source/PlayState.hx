...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		FG = new FlxTypedGroup<BGSprite>(20);

		switch (curStage)
		{
			case 'video':
				var wall:BGSprite = new BGSprite('arcade/bg/wall', -800, -540, 0.62, 1);
				wall.scale.set(0.77, 0.77);
				wall.updateHitbox();
				add(wall);
		...
		var toMiddle = ClientPrefs.middleScroll && SONG.needsVoices;
		strumLine = new FlxSprite(toMiddle ? STRUM_X_MIDDLESCROLL : STRUM_X, 50).makeGraphic(FlxG.width, 10);
		strumUpY = strumLine.y + 70;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					{
						if (loadOnto.length > 0)
							oldNote = loadOnto[Std.int(loadOnto.length - 1)];
						else
							oldNote = null;
					}
					...
					var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, secNum, songNotes[2]);
					swagNote.y = 300;
					...
					var wholeNote = swagNote;
					loadOnto.push(swagNote);
					unspawnNotes.push(wholeNote);

					var floorSus:Int = Math.floor(susLength);
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							if (prevLoaded) oldNote = unspawnNotes[Std.int(unspawnNotes.length - 1)];
							else oldNote = loadOnto[Std.int(loadOnto.length - 1)];
							...
							else if(ClientPrefs.middleScroll && SONG.needsVoices)
							{
								sustainNote.x += 310;
								if(daNoteData > 1)
								{ //Up and Right
		...
		//trace(loadOnto.length);
		//unspawnNotes = unspawnNotes.concat(loadOnto);
		/*
		for (i in 0...loadOnto.length)
		{
			unspawnNotes.push(loadOnto[i]);
		}
		*/
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1 && ClientPrefs.middleScroll &&  SONG.needsVoices) targetAlpha = 0.5;

			var bbY = strumLine.y;
			if (player == 0 && ClientPrefs.downScroll) bbY = strumUpY;

			var toMiddle = ClientPrefs.middleScroll && SONG.needsVoices;
			var babyArrow:StrumNote = new StrumNote(toMiddle ? STRUM_X_MIDDLESCROLL : STRUM_X, bbY, i, player);
			if (!isStoryMode)
			{
				babyArrow.y -= 10;
				babyArrow.alpha = 0;
			...
			{
				if(ClientPrefs.middleScroll && SONG.needsVoices)
				{
					babyArrow.x += 310;
					if(i > 1) { //Up and Right
	...
	override public function update(elapsed:Float)
	{
		...
		if (unspawnNotes[0] != null)
		{
			...
			for (i in 0...unspawnNotes.length)//while (unspawnNotes.length > 0 && unspawnNotes[0].strumTime - Conductor.songPosition < time)
			{
				var dunceNote:Note = unspawnNotes[i];
				if (dunceNote.isSustainNote) notes.add(dunceNote);
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				var memb = playerStrums.members[daNote.noteData];
				if (!daNote.mustPress) memb = opponentStrums.members[daNote.noteData];
				
				strumX = memb.x;
				strumY = memb.y;
				strumAngle = memb.angle;
				strumAlpha = memb.alpha;

				strumX += daNote.offsetX;
				strumY += daNote.offsetY;
				strumAngle += daNote.offsetAngle;
				strumAlpha *= daNote.multAlpha;
				var center:Float = memb.getMidpoint().y - 16;

				if(daNote.copyX) {
					daNote.x = strumX;
				}
				if(daNote.copyAngle) {
					daNote.angle = strumAngle + daNote.pointyAngle;
				}
				if(daNote.copyAlpha) {
					daNote.alpha = strumAlpha;
				}
				if(daNote.copyY) {
					if (ClientPrefs.downScroll && daNote.mustPress) {
						daNote.y = (strumY + 0.45 * (Conductor.songPosition - daNote.strumTime) * roundedSpeed);
						if (daNote.mainSus) daNote.y += Note.swagWidth / 2;
				...
				var doKill:Bool = daNote.y < - daNote.frameHeight;
				if(ClientPrefs.downScroll && daNote.mustPress) doKill = daNote.y > FlxG.height;

				if (daNote.mainSus)
				{
					doKill = daNote.y < -daNote.frameHeight * daNote.scale.y;
					if (ClientPrefs.downScroll && daNote.mustPress) doKill = daNote.y > FlxG.height + daNote.frameHeight * Math.abs(daNote.scale.y);
				}
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == "revolution" && ending)
					{
						...
						bs.cameras = [camHUD];
						add(bs);
						FlxG.save.data.ending[Std.parseInt(isPointyAngry)][Std.parseInt(isPlayerBad)] = true;
						FlxG.save.flush();
	...
	override function beatHit()
	{
		...
		strumLineNotes.forEach(function(n:StrumNote)
		{
			if (n.animation.curAnim.name == 'static') n.playAnim('static', true);
		});
